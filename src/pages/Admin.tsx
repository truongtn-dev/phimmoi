...
import { Trash2, Edit, Plus, Film, Users } from "lucide-react";
      ...
      <div className="pt-24 px-6 sm:px-12 max-w-6xl mx-auto">
        ...
        <div className="flex gap-2 mb-6">
          <Button
            ...
            className={tab === "movies" ? "bg-primary text-primary-foreground" : "border-border text-foreground"}
          >
          ...
          <Button
            ...
            className={tab === "users" ? "bg-primary text-primary-foreground" : "border-border text-foreground"}
          >
              ...
              <div className="bg-card border border-border rounded-lg p-6 mb-6 animate-scale-in space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="bg-secondary" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">Category</label>
                    <Input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="bg-secondary" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">Poster URL</label>
                    <Input value={form.poster_url} onChange={(e) => setForm({ ...form, poster_url: e.target.value })} className="bg-secondary" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">Trailer URL</label>
                    <Input value={form.trailer_url} onChange={(e) => setForm({ ...form, trailer_url: e.target.value })} className="bg-secondary" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">Rating</label>
                    <Input type="number" step="0.1" min="0" max="10" value={form.rating} onChange={(e) => setForm({ ...form, rating: e.target.value })} className="bg-secondary" />
                  </div>
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-secondary border border-border rounded-md p-2 text-sm text-foreground min-h-[80px]"
                  />
                </div>
                <div className="flex gap-2">
                  <Button onClick={handleSaveMovie} className="bg-primary text-primary-foreground">
                    {editId ? "Update" : "Add"}
                  </Button>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {movies.data?.map((movie) => (
                    <tr key={movie.id} className="border-b border-border/50 hover:bg-muted/30">
                      ...
                      <td className="p-3 text-right">
                        <button onClick={() => handleEdit(movie)} className="p-1.5 hover:text-primary transition-colors">
                          <Edit className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDeleteMovie(movie.id)} className="p-1.5 hover:text-destructive transition-colors">
                  ...
                  {movies.data?.length === 0 && (
                    <tr><td colSpan={4} className="p-6 text-center text-muted-foreground">No movies yet</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  <th className="text-left p-3 text-muted-foreground font-medium hidden sm:table-cell">Joined</th>
                </tr>
              </thead>
              <tbody>
